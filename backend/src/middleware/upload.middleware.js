...
export const handleMulterError = (err, req, res, next) => {
  if (err instanceof multer.MulterError) {
    if (err.code === 'LIMIT_FILE_SIZE') {
      const maxImageMb = Math.round(MAX_IMAGE_SIZE / (1024 * 1024));
      const maxPdfMb = Math.round(MAX_PDF_SIZE / (1024 * 1024));
      return next(new ApiError(400, `File too large. Maximum size is ${maxImageMb}MB for images, ${maxPdfMb}MB for PDFs.`));
    }
